...
| :-: | ------ | :-: | ------------------------------------------------------------ | ---------------- | -------------------------------- |
|  [x]  | 1A.1   | P1  | 크롤러: **coos.kr + Paula's Choice** + INCI 정규화·중복제거            | 0.5              | 실데이터 적재, 출처·수집일시 기록              |
|  [x]  | 1A.2   | P1  | 문서·제품 임베딩→pgvector + 유사도 검색(`documents/search.py`)           | 1A.1             | top-k 유사도 비어있지 않음 (AC-D1)        |
|  [x]  | 1A.3   | P1  | [SKIP] **AC-D1 제형토큰 실측 게이트(≥60%)** (2026-07-08 합의 스킵: 실제품 설명에 제형 단어가 이미 풍부하여 스킵 결정) | 1A.2             | 커버리지 측정 리포트 생략 (검증 무관)          |
|  [x]  | 1A.4   | P1  | 지식 엣지 적재: 문서·크롤에서 `TREATS/AGGRAVATES`·`HELPS/CONFLICTS`를 그래프에 직접 추출·적재 | 0.5, 1A.1        | 그래프 지식 엣지 non-empty              |
|  [x]  | 1A.5   | P1  | 성분→제품 조회 + **회피성분 완전차단 필터**(관계형)                             | 1A.1             | AC-D2/R2                         |
...
| ☐ | ID | P | 할 일 | 선행 | 완료 조건(요약) |
|:-:|---|:-:|---|---|---|
| [x] | 1B.1 | P1 | LLM 껍데기 구현 + fact 추출기(중요도 거르기) | 0.1 | AC-M3 |
| [x] | 1B.2 | P1 | CRUD 판정기(추가/수정/삭제/무시, delete=soft+감사) | 1B.1 | AC-M1 |
| [x] | 1B.3 | P0 | 중요도 점수(**λ=0.05**) + 개인격리 저장(RLS) + `rank_memory` 제공 | 0.2 | AC-M2/M5 |
| [x] | 1B.4 ⭐ | P0 | **`writer.py` 동기 원자 저장**(표+벡터+그래프 한 묶음, 실패 전부취소, advisory lock) | 0.2, 1B.2 | **AC-S1/S2** |
| [x] | 1B.5 ⭐ | P0 | **기억→그래프 다리**(개인 회피/선호 연결선, choke 경유, writer tx 안) | 1B.4, 0.5 | AC-G2(개인 엣지) |
| [x] | 1B.6 | P1 | 대화 총괄(구체/모호 분기 + 좁혀가기 퍼널 + 근거 생성) — fixture로 개발 | 0.3 | AC-R1/R3/R4/M4 |
| [x] | 1B.7 | P1 | 기억 테스트(AC-M1~M6, 중요도 평가셋 ≥20개) — **개발과 병행** | 1B.2 | CI 통과 |

### Phase 2 — 합치고 검증 (공동, 전부 ⭐)

| ☐ | ID | P | 할 일 | 선행 | 확인(AC) |
|:-:|---|:-:|---|---|---|
| [x] | 2.1 ⭐ | P0 | fixture→진짜 검색 교체 + 배선 통합(대화↔검색↔저장, `app/`) | 1A.7, 1B.6, 1B.4 | 계약 맞물림·AC-M4 |
| [x] | 2.2 ⭐ | P1 | **최근성 루프**(쓴 즉시 다음 턴 반영, drain 없음) | 2.1 | AC-M6 |
| [x] | 2.3 ⭐ | P0 | **격리 테스트 0건**(남의 기억·그래프 조회) | 1A.6, 1B.3 | AC-M5/G3 |
| [x] | 2.4 ⭐ | P0 | **원자성 결함주입 테스트**(그래프 쓴 뒤 커밋 전 실패→3저장 0잔여) | 1B.4 | AC-S1 |
| [x] | 2.5 ⭐ | P1 | 대표 시나리오 E2E("가을 건조·오일싫음·에멀전") 4단언 통과 | 2.1 | AC-R4 |
| [x] | 2.6 ⭐ | P1 | 최종 점검: 회피 0건·근거 단언 정합·제형 랭킹 + 성능 예산(AGE 순회·p95) | 2.5 | AC-R2/R3/F1·예산 |

> **2.6 성능 예산 알려진 한계**: 검색(비-LLM) p95=108ms, AGE 순회 p95=2.8ms — 예산(≤400ms) 통과.
> 종단(AI 포함) p95=9.85s(NVIDIA gpt-oss-120b 기준)는 예산(≤2.5s) 미달성. 원인은 (1) 한 턴에
> LLM을 최대 3회 연속 호출하는 구조, (2) 측정에 쓴 NVIDIA Build 무료 티어의 변동성 — 실제
> 운영 프로바이더(Gemini)로 재측정 및 개선(호출 병렬화·횟수 축소 등)은 후속 과제로 남김.
