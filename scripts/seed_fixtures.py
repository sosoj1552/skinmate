...
def main() -> None:
    ...
    with psycopg.connect(db_url) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                "TRUNCATE TABLE memories, memory_audit, product_ingredients, ingredients, products, documents CASCADE;"
            )
            ...
            cur.execute(
                """
                INSERT INTO ingredients (canonical_key, name_ko, name_en, inci_key, grade, function, classification)
                VALUES 
                ('hyaluronic_acid', '히알루론산', 'Hyaluronic Acid', 'hyaluronic acid', 'Good', '보습제', '보습'),
                ('retinol', '레티놀', 'Retinol', 'retinol', 'Best', '피부컨디셔닝제', '안티에이징'),
            ...
            cur.execute(
                """
                INSERT INTO products (name, brand, category, description, embedding, embedding_model_id)
                VALUES 
                ('수분 에멀전', 'coos', '에멀전', '끈적임 없는 가벼운 제형의 고보습 에멀전. 히알루론산이 풍부하게 함유되어 속건조를 해결.', %s, 'bge-m3'),
                ('레티놀 0.1 세럼', 'paulas-choice', '세럼', '주름 개선을 돕는 고농축 레티놀 세럼. 민감성 피부는 자극 주의.', %s, 'bge-m3')
            ...
            cur.execute(
                ...
                VALUES (
                    '가을철 환절기에는 대기 중 습도가 급격히 낮아져 피부 장벽이 약화되고 속건조가 심해집니다. 이 시기에는 히알루론산과 같이 수분을 강력하게 끌어당기는 성분이 함유된 에멀전 제형을 사용하는 것이 좋습니다. 반면, 무거운 페이스 오일은 민감하거나 지성 피부에 끈적임과 자극을 유발할 수 있으므로 피하는 것이 안전합니다.',
                    %s, 'bge-m3',
                    '{"url": "https://example.com/skincare/autumn-guide", "kind": "beautypedia_prose", "crawled_at": "2026-06-30T10:00:00Z", "robots_ok": true}'::jsonb
                );
                """,
                (dummy_vector,),
            )
            ...
            cur.execute(
                f"""
                INSERT INTO memories (user_id, content, fact_type, target_ingredient_id, target_name, season) VALUES
                (1001, '가을철 건조함이 고민이에요', 'has_concern', NULL, 'dryness', '가을'),
                (1001, '레티놀 성분을 바르면 자극이 있고 붉어져서 피하고 싶어요', 'avoid_ingredient', {ing_map['retinol']}, NULL, NULL),
                (1002, '피부가 얇고 아주 민감해요', 'skin_type', NULL, NULL, NULL),
                (1002, '히알루론산이 들어간 제품은 촉촉하고 잘 맞아요', 'prefer_ingredient', {ing_map['hyaluronic_acid']}, NULL, NULL);
                """
            )
            ...
            cur.execute(
                """
                SELECT * FROM cypher('skinmate', $$
                    MATCH (p1:Product {product_id: 10}), (i1:Ingredient {canonical_key: 'hyaluronic_acid'})
                    MATCH (p2:Product {product_id: 11}), (i2:Ingredient {canonical_key: 'retinol'})
                    MATCH (p3:Product {product_id: 11}), (i3:Ingredient {canonical_key: 'alcohol'})
                    MATCH (i1_t:Ingredient {canonical_key: 'hyaluronic_acid'}), (c_d:Concern {name: 'dryness'})
                    MATCH (i2_t:Ingredient {canonical_key: 'retinol'}), (c_d2:Concern {name: 'dryness'})
